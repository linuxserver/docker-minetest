...
You can add the following optional parameters to the `docker create` command.

* `-e game_git_repo_url=<git https URL>` - Use an other subgame than the default (minetest)
* `-e default_password=<anypassword>` - Choose a default password for new users
* `-e world=<worldname>` - Choose the world name, world by default

## Parameters

`The -v and -e parameters are split into two halves, separated by a colon, the left hand side representing the host and the right the container side. 
